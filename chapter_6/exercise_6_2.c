...
int keytab_len = sizeof(keytab) / sizeof(keytab[0]);
...
main()
{
    ...
    while (getword(word, MAXWORD) != EOF)
    {
        if (isalpha(word[0]))
        {
            ...
            {
                if (string_in_string_array(word, variable_types, (sizeof(variable_types) / sizeof(variable_types[0]))) >= 0)
                {
                    do
                    {
...
int string_in_string_array(char *str, char *str_arr[], int len)
{
    for (int i = 0; i < len; i++)
    {
